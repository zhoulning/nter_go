...
import { api } from '../api'
import type { Opportunity, PredictionInfo } from '../types'
import { DIFFICULTY_META, PREDICT_GROUPS, ROUND_LABEL } from '../types'
...
async function generate() {
  ...
  try {
    ...
    message.success(`「${ROUND_LABEL[saved.round_type] ?? '面试'}」题单已生成（${saved.question_count} 题）`)
  } catch (e) {
...
function removePrediction() {
  ...
  dialog.warning({
    title: '删除题单',
    content: `确定删除「${ROUND_LABEL[target.round_type] ?? '面试'}」的预测题单吗？`,
    positiveText: '删除',
    negativeText: '取消',
  ...
  <div class="max-h-[calc(100vh-180px)] overflow-y-auto pb-8">
    ...
    <div v-if="generating" class="grid place-items-center rounded-2xl border border-dashed border-indigo-200 bg-white py-16">
      <div class="text-center">
        <n-spin size="28" />
        <div class="mt-3 text-[14px] font-semibold text-zinc-700">正在生成「{{ ROUND_LABEL[selectedRoundType] ?? '面试' }}」预测题单…</div>
        <p class="mt-1 text-[12.5px] text-zinc-400">AI 正在结合 JD、简历、匹配度缺口与题库弱项出题，大约需要一分钟</p>
      </div>
    </div>
    ...
    <div v-else-if="!predictions.length && !loading" class="grid place-items-center rounded-2xl border border-dashed border-zinc-200 bg-white py-16">
      <div class="w-[420px] text-center">
        ...
        <p class="mt-1 text-[12.5px] leading-relaxed text-zinc-400">
          基于 JD、关联简历、匹配度报告缺口与题库弱项，按目标轮次生成分维度题单（附考察意图与答题要点）
        </p>
        <div class="mt-4 flex items-center justify-center gap-2">
          ...
          >
            <option v-for="(label, key) in { written: '笔试', first: '一面', second: '二面', third: '三面', cross: '交叉面', hr: 'HR 面', other: '其他' }" :key="key" :value="key">
              {{ label }}
            </option>
          </select>
    ...
    <template v-else>
      ...
      <div class="mb-3 flex flex-wrap items-center gap-2">
        ...
        >
          {{ ROUND_LABEL[p.round_type] ?? p.round_type }}
